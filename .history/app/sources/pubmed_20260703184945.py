...
async def search(keyword: str, retmax: int = 15) -> list:
    ...
    Uses PubMed boolean syntax with [Title/Abstract] field tags:
      ([journal filter]) AND (keyword[Title/Abstract])

    This targets title & abstract specifically, not author names / MeSH / etc.
    """
    headers = {"User-Agent": "ScholarPulse/1.0"}

    # Build boolean query: [journal filter] AND (keyword in Title/Abstract)
    jf = _build_journal_filter()
    kw_tagged = f'"{keyword}"[Title/Abstract]' if " " in keyword else f'{keyword}[Title/Abstract]'
    term = f"{jf} AND {kw_tagged}" if jf else kw_tagged
