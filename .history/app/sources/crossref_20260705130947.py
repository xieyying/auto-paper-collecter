...
def _get_issn_for_journal(jn: str) -> tuple[str, str]:
    """Resolve a journal name to its (pISSN, eISSN). Both may be empty."""
    return _pm._get_all_issns(jn)
    ...
    async with httpx.AsyncClient(timeout=30, follow_redirects=True) as c:
        if has_journals:
            ...
            async def _fetch_one(jn: str) -> list:
                async with sem:
                    p_issn, e_issn = _get_issn_for_journal(jn)
                    issns_to_try = [i for i in (p_issn, e_issn) if i]
                    if not issns_to_try:
                        return []
                    seen_local: set[str] = set()
                    parsed = []
                    for issn in dict.fromkeys(issns_to_try):  # deduplicate
                        raw_items = await _fetch_journal(c, keyword, issn, rows, headers)
                        for it in raw_items:
                            item = _parse_crossref_item(it)
                            if item is None:
                                continue
                            dedup_key = item.doi or item.ext_id
                            if dedup_key and dedup_key in seen_local:
                                continue
                            if dedup_key:
