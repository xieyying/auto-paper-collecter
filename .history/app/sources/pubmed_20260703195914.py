...
def _parse_pubmed_xml(xml: str, expected_ids: list[str]) -> list:
    ...
    for block in articles:
        ...
        pub_date = None
        # Extract raw Year/Month/Day directly from the PubmedArticle block
        y_str = _extract_raw(block, "Year")
        m_str = _extract_raw(block, "Month")
        d_str = _extract_raw(block, "Day")

        # Only use Year from PubDate (not DateCompleted/DateRevised).
