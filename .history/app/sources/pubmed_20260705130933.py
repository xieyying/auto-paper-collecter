...
async def search(keyword: str, retmax: int = 50,
    ...
    if journal_list:
        # Resolve journal names to all available ISSNs (pISSN + eISSN)
        from ..impact import get_impact_factor
        all_issns: list[str] = []
        for jn in journal_list:
            p, e = _get_all_issns(jn)
            if p:
                all_issns.append(p)
            if e and e != p:
                all_issns.append(e)
        # Deduplicate and sort by IF descending
        seen = dict.fromkeys(all_issns)
        valid = {issn for issn in seen if issn and issn != "N/A"}
    ...
    async with httpx.AsyncClient(timeout=30, follow_redirects=True) as c:
        if issns:
            # Single OR query across all journals — avoids 429 rate limit
