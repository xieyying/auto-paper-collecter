...
async def search(keyword: str, retmax: int = 50) -> list:
    """Search PubMed by keyword across ALL fields.

    If PUBMED_JOURNALS is set, searches each journal individually (like ACS/Nature
    do via Crossref) so each journal gets its own allocation of results.
    Otherwise searches all PubMed at once.
    """
    headers = {"User-Agent": "ScholarPulse/1.0"}
    items: list[RawItem] = []

    # Determine per-journal cap
    journals = [_JOURNAL_ABBR.get(j, j) for j in PUBMED_JOURNALS] if PUBMED_JOURNALS else []
    per_journal = max(3, retmax // len(journals)) if journals else retmax

    async with httpx.AsyncClient(timeout=30, follow_redirects=True) as c:
        if journals:
            # Search each journal individually (like ACS/Nature)
            for j in journals:
                term = f'"{j}"[ta] AND ({keyword})'
                batch = await _search_and_fetch(c, term, per_journal, headers)
                items.extend(batch)
        else:
            # No journal filter — search all PubMed
            term = keyword
