...
async def search(keyword: str, retmax: int = 50) -> list:
    ...
    async with httpx.AsyncClient(timeout=30, follow_redirects=True) as c:
        ...
        # ── Journal filter ──────────────────────────────────────
        journal_list = _pm._ACTIVE_JOURNALS if _pm._ACTIVE_JOURNALS is not None else _pm.PUBMED_JOURNALS
        issns: list[str] = []
        if journal_list:
            from ..impact import get_impact_factor
            all_issns: list[str] = []
            for jn in journal_list:
                issn = _pm._get_issn(jn) or _pm._get_eissn(jn)
                if issn:
        ...
        base_term = f'all:"{keyword}"'

        async def _fetch_one_pmc(issn: str) -> list:
            """Search PMC for a single journal by ISSN."""
            term = f'"{issn}"[issn] AND ({base_term})'
            params = {
                "db": "pmc",
                "term": term,
                "retmax": str(max(3, retmax // len(issns))) if issns else str(retmax),
                "retmode": "json",
                "email": NCBI_EMAIL,
            }
            try:
                ...
                print(f"[pmc] esearch error for {issn}: {e}", flush=True)
                return []
            ...
            for attempt in range(3):
                try:
                    ...
                    if attempt == 2:
                        print(f"[pmc] efetch error for {issn}: {e}", flush=True)
                        return []
        ...
        if issns:
            ...
            async def _fetch_one_sem(issn: str) -> list:
                async with sem:
                    return await _fetch_one_pmc(issn)

            results = await asyncio.gather(*[_fetch_one_sem(i) for i in issns])
            ...
            # No journal filter — single query
            params = {
                "db": "pmc",
                "term": base_term,
                "retmax": str(retmax),
...
def _parse_pmc_xml(xml: str, expected_ids: list[str]) -> list:
    ...
    for i, block in enumerate(articles[1:], 1):  # skip everything before first article
        if not block.strip():
            continue
        # Get PMC ID from article-meta (strip "PMC" prefix)
        pmcid = _ptag(block, 'article-id', 'pub-id-type', 'pmcid').strip()
        pmid = _ptag(block, 'article-id', 'pub-id-type', 'pmid')
        raw_id = pmcid.replace("PMC", "").strip() if pmcid else (pmid or "").strip()
        if not raw_id or raw_id not in expected_ids:
            continue
        ...
        # Date from <pub-date> — extract raw without cleaning tags
        pub_date = None
        pub_m = re.search(r"<pub-date[^>]*>(.*?)</pub-date>", block, re.S)
        if pub_m:
