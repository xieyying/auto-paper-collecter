...
# ── Journal filter ──────────────────────────────────────────────
# Default journal list (used when settings haven't been saved yet).
# Journals are managed from the subscription settings UI.
...
_PUBMED_EISSN_MAP: dict[str, str] = {}  # journal name (lower) -> eISSN
_JOURNAL_ISSN_PAIRS: dict[str, tuple[str, str]] = {}  # journal name (lower) -> (pISSN, eISSN)
try:
    import openpyxl
    ...
    if os.path.isfile(_if_path):
        ...
        for _row in _ws.iter_rows(min_row=2, values_only=True):
            ...
            if _issn and _abbr:
                ...
                _JOURNAL_ISSN_PAIRS[_abbr] = (_issn, _eissn)
            if _eissn and _abbr:
                _PUBMED_ISSN_MAP[_eissn.lower()] = _eissn
                _PUBMED_EISSN_MAP[_abbr] = _eissn
            if _eissn and _full:
                _PUBMED_ISSN_MAP[_full] = _eissn
            if _issn and _full:
                _PUBMED_ISSN_MAP[_full] = _issn
        _wb.close()
        print(f"[pubmed] loaded {len(_PUBMED_ISSN_MAP)} ISSN mappings, {len(_JOURNAL_ISSN_PAIRS)} journal pairs", flush=True)
except Exception:
    pass
...
# Fallback: journal name -> eISSN
_FALLBACK_EISSN: dict[str, str] = {
    "nature": "1476-4687", "nat biotechnol": "1546-1696", "nat chem biol": "1552-4469",
    "nat commun": "2041-1723", "nat methods": "1548-7105", "nat microbiol": "2058-5276",
    "science": "1095-9203", "cell": "1097-4172",
    "j am chem soc": "1520-5126", "jacs au": "2691-3704",
    "acs synth biol": "2161-5063", "acs chem biol": "1554-8937",
    "biochemistry": "1520-4995", "acs cent sci": "2374-7951",
    "j med chem": "1520-4804", "acs catal": "2155-5435",
    "org lett": "1523-7052", "j org chem": "1520-6904",
    "anal chem": "1520-6882", "j agric food chem": "1520-5118",
    "j nat prod": "1520-6025",
    "angew chem int ed engl": "1521-3773",
    "j antibiot": "1881-1469",
    "j chem inf model": "1549-960X",
}


def _get_issn(name: str) -> str:
    """Resolve a journal name to its print ISSN (pISSN)."""
...
def _get_eissn(name: str) -> str:
    """Resolve a journal name to its electronic ISSN (eISSN)."""
    key = name.strip().lower()
    if key in _PUBMED_EISSN_MAP:
        return _PUBMED_EISSN_MAP[key]
    if key in _FALLBACK_EISSN:
        return _FALLBACK_EISSN[key]
    return ""


def _get_all_issns(name: str) -> tuple[str, str]:
    """Resolve a journal name to (pISSN, eISSN). Both may be empty."""
    key = name.strip().lower()
    p = _get_issn(name)
    e = _get_eissn(name)
    return (p, e)
...
# Dynamic journal list — set via set_journals() before search
_ACTIVE_JOURNALS: list[str] | None = None


def set_journals(journals: list[str] | None) -> None:
    """Set the journal list for subsequent searches. None = all PubMed."""
    global _ACTIVE_JOURNALS
    _ACTIVE_JOURNALS = journals
...
async def search(keyword: str, retmax: int = 50,
                 journals: list[str] | None = None) -> list:
    """Search PubMed by keyword.

    Args:
        keyword: Search query.
        retmax: Max results (when no per-journal filtering).
        journals: List of journal names to restrict to.
                  None or empty = search all PubMed.
    """
    headers = {"User-Agent": "ScholarPulse/1.0"}
    items: list[RawItem] = []

    journal_list = _ACTIVE_JOURNALS if _ACTIVE_JOURNALS is not None else PUBMED_JOURNALS
    if journal_list:
        # Resolve journal names to ISSNs, sorted by IF descending
        from ..impact import get_impact_factor
        all_issns: list[str] = []
        for jn in journal_list:
            issn = _get_issn(jn) or _get_eissn(jn)
            if issn:
                all_issns.append(issn)
        valid = {issn for issn in all_issns if issn and issn != "N/A"}
        issn_if = [(issn, get_impact_factor(issn) or 0) for issn in valid]
    ...
    async with httpx.AsyncClient(timeout=30, follow_redirects=True) as c:
        if issns:
            # Per-journal iteration — guarantees every journal contributes results
            sem = asyncio.Semaphore(3)

            async def _fetch_one(issn: str) -> list:
                async with sem:
                    term = f'"{issn}"[issn] AND ({keyword})'
                    return await _search_and_fetch(c, term, per_journal, headers)

            results = await asyncio.gather(*[_fetch_one(i) for i in issns])
            seen_ids: set[str] = set()
            for chunk in results:
                for item in chunk:
                    if item.ext_id not in seen_ids:
                        seen_ids.add(item.ext_id)
                        items.append(item)
        else:
