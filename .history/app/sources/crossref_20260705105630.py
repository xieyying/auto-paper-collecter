...
import httpx
from . import RawItem
from .pubmed import _ACTIVE_JOURNALS, PUBMED_JOURNALS, _PUBMED_ISSN_MAP
...
def _journal_matches(venue: str, issn: str) -> bool:
    ...
    jlist = _ACTIVE_JOURNALS if _ACTIVE_JOURNALS is not None else PUBMED_JOURNALS
    if not jlist:
        return True  # empty list = all journals
    # Check ISSN match
    if issn:
        for jn in jlist:
            j_issn = _PUBMED_ISSN_MAP.get(jn.lower())
