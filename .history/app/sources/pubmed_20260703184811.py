...
async def search(keyword: str, retmax: int = 15) -> list:
    """Search PubMed by keyword, optionally restricted to PUBMED_JOURNALS.

    Uses PubMed boolean syntax with [Title/Abstract] field tags:
      ([journal filter]) AND (keyword1[Title/Abstract] OR keyword2[Title/Abstract] ...)

    This ensures the search targets title & abstract (not author names, MeSH, etc.).
    """
    headers = {"User-Agent": "ScholarPulse/1.0"}

    # ── Build boolean query ────────────────────────────────────
    # Split the keyword into individual phrases (PubMed understands quoted phrases)
    # and add [Title/Abstract] to each, then OR them.
    # This preserves PubMed's native boolean search capability.
    phrases = [kw.strip() for kw in keyword.split(" AND ")]
    kw_parts = []
    for p in phrases:
        # If the query contains OR, wrap each sub-term individually
        sub_terms = [t.strip() for t in p.split(" OR ")]
        or_parts = [f'"{t}"[Title/Abstract]' if " " in t and not t.startswith('"')
                    else f'{t}[Title/Abstract]' for t in sub_terms]
        kw_parts.append(" OR ".join(or_parts))

    # Combine: journal filter AND (keyword fields)
    jf = _build_journal_filter()
    if jf:
        term = f"{jf} AND ({kw_parts[0]})" if len(kw_parts) == 1 else \
               f"{jf} AND ({' AND '.join(kw_parts)})"
    else:
        term = kw_parts[0] if len(kw_parts) == 1 else " AND ".join(kw_parts)
