...
def _parse_pubmed_xml(xml: str, expected_ids: list[str]) -> list:
    ...
    for block in articles:
        ...
        pub_date = None
        pub_block = _extract_tag(block, "PubDate")
        if pub_block:
            # Extract Year/Month/Day from within the PubDate block
            y_str = _extract_tag_in(pub_block, "Year")
            m_str = _extract_tag_in(pub_block, "Month")
            d_str = _extract_tag_in(pub_block, "Day")
            if y_str:
                try:
                    y = int(y_str)
                    m = 1
                    ...
                    ym = re.search(r"(\d{4})", pub_block)
                    if ym:
                        pub_date = dt.datetime(int(ym.group(1)), 1, 1)

        # Fallback: if PubDate only has MedlineDate (e.g. "2026 Mar-Apr")
        if not pub_date:
            medline = _extract_tag_in(pub_block, "MedlineDate") if pub_block else ""
            if medline:
        ...
        # Journal — extract the Title element specifically, not the whole <Journal> block
...
def _extract_tag_in(xml: str, tag: str) -> str:
    """Extract tag content from a smaller XML fragment (avoids matching outer tags)."""
    m = re.search(rf"<{tag}[^>]*>(.*?)<\/{tag}>", xml, re.S)
    return _clean(m.group(1)) if m else ""
