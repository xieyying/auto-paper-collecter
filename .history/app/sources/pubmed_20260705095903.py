...
async def search(keyword: str, retmax: int = 50) -> list:
    ...
    issn_if = [(issn, get_impact_factor(issn) or 0) for issn in
              dict.fromkeys(_get_issn(j) for j in PUBMED_JOURNALS if _get_issn(j))]
    issn_if.sort(key=lambda x: -x[1])  # highest IF first
    ...
    async with httpx.AsyncClient(timeout=30, follow_redirects=True) as c:
        if issns:
            for i, issn in enumerate(issns):
                ...
                items.extend(batch)
                # Polite delay between per-journal searches to avoid 429
                if i < len(issns) - 1:
