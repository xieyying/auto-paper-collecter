...
def _journal_matches(venue: str, issn: str) -> bool:
    ...
    # Check ISSN match (most reliable)
    if issn:
        for jn in jlist:
            j_issn = _pm._PUBMED_ISSN_MAP.get(jn.lower())
            if j_issn and issn.strip().lower() == j_issn.lower():
                return True
    # Check venue name match (full word boundary, not loose substring)
    vl = venue.lower().strip()
    for jn in jlist:
        jnl = jn.lower()
        # Exact match (e.g. "Nature" == "Nature")
        if jnl == vl:
            return True
        # Venue starts with journal name (e.g. "Nature Biotechnology" starts with "Nature")
        if vl.startswith(jnl) and (len(vl) == len(jnl) or vl[len(jnl)] in ' &,;:'):
            return True
        # Journal name starts with venue or contains it as a whole word
        if jnl.startswith(vl) and (len(jnl) == len(vl) or jnl[len(vl)] in ' &,;:'):
            return True
    return False


async def search(keyword: str, rows: int = 200):
    params = {
        "query": keyword,
        "sort": "published",
        "order": "desc",
