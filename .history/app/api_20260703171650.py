...
def bootstrap(db: Session = Depends(get_db)):
    ...
    src_objs = [{"name": n, "color": SRC_STYLE.get(n, SRC_STYLE["Crossref"])["color"],
                 ...
                 "desc": {"arXiv": "预印本", "Crossref": "期刊/会议元数据",
                          ...
                          "学术新闻": "RSS 动态",
                          "Nature": "Nature 系列期刊", "ACS": "JACS/ACS SynBio 等"}.get(n, "")}
...
async def deep_search_endpoint(body: dict = Body(...)):
    """Deep search: AND-combined expanded keywords with time range.

    Request body:
      keywords: list[str] — up to 3 keywords
      time_range_days: int — look-back window (default 365)
      sources: dict[str,bool] | null — per-source enable map

    Returns list of result dicts with TLDR summaries.
    """
    keywords = body.get("keywords", [])
    time_range = body.get("time_range_days", 365)
    sources = body.get("sources")
    results = await deep_search(keywords, time_range, sources)
