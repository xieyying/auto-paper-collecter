...
def get_impact_factor(journal: str) -> float | None:
    ...
    # 3) Substring match — but filter out obviously wrong matches
    #    (e.g. "j agric food chem" matching "british journal of...").
    #    Only return if key is a SHORT abbreviation-like string.
    for key, val in _IF_MAP.items():
        if key in j or j in key:
            if len(key) < 30:  # short keys are abbreviations, more reliable
                return val
    return None
