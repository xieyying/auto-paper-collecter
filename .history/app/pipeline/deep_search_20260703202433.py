...
async def deep_search(keywords: list[str], time_range_days: int = 365,
    ...
    # Before dedup, preserve keyword matches across duplicate copies
    kw_map: dict[tuple, list[str]] = {}
    from .dedup import _key as dedup_key
    for it in all_items:
        k = dedup_key(it)
        existing = kw_map.setdefault(k, [])
        for kw in getattr(it, "_kw_matches", []):
            if kw not in existing:
                existing.append(kw)

    all_items = dedup(all_items)

    # Restore merged keyword matches after dedup
    for it in all_items:
        k = dedup_key(it)
        merged = kw_map.get(k, [])
        if merged:
            it._kw_matches = merged
    ...
    for it in all_items:
        ...
        # Filter out the original keywords, keep only expanded queries
        expanded = [q for q in matches if q not in keywords]
        # If only original keywords matched, show them; otherwise show expanded queries
        display = expanded if expanded else matches
