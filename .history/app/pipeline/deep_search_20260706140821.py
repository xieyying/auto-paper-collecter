...
async def deep_search(keywords: list[str], time_range_days: int = 365,
    ...
    #     Check that the ORIGINAL keyword (what the user typed) appears as a
    #     phrase in the paper's title/abstract.  This removes false positives
    #     where the search API returned an irrelevant paper that happened to
    #     match an expanded query but has nothing to do with the user's intent.
    #     e.g. user typed "protein language model" → paper must contain that
    #     exact phrase (or its lowercase version) in title or abstract.
    if keywords and all_items:
        filtered = []
        kw_lower = [k.lower() for k in keywords]
        for it in all_items:
            text = f"{it.title or ''} {it.abstract or ''}".lower()
            # Count how many of the user's original keywords appear in text
            matched = sum(1 for kw in kw_lower if kw in text)
            # Keep if at least one keyword matched (lenient),
            # or if the AND filter already ensured cross-group match
            if matched > 0:
                filtered.append(it)
        if filtered:
            before = len(all_items)
            all_items = filtered
            print(f"[deep-search] text filter: {before} -> {len(filtered)} (dropped {before-len(filtered)} with no keyword in text)", flush=True)
