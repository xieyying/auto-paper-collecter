...
def _matches_keywords(text: str, kw_tokens: list[set[str]]) -> tuple[int, int]:
    """Return (matched_count, total_needed) — how many keywords are matched."""
    if not text:
        return (0, len(kw_tokens))
    tokens = set(_TOK.findall(text.lower()))
    matched = sum(1 for needed in kw_tokens if not needed.isdisjoint(tokens))
...
async def deep_search(keywords: list[str], time_range_days: int = 365,
    ...
    if len(kw_tokens) > 1:
        ...
        for it in all_items:
            matched, total = _matches_keywords(f"{it.title} {it.abstract}", kw_tokens)
    ...
    for it in all_items:
        ...
        matched_kws = [kw for kw, tokens in zip(keywords, kw_tokens)
                       if _matches_keywords(text, [tokens])[0] > 0]
