...
async def deep_search(keywords: list[str], time_range_days: int = 365,
    ...
    if all_items and expanded_per_kw:
        _TOK2 = re.compile(r"[a-z0-9]{3,}")
        kw_query_sets = [set(qs) for qs in expanded_per_kw]
        filtered = []
        for it in all_items:
            text = f"{it.title or ''} {it.abstract or ''}".lower()
            text_tokens = set(_TOK2.findall(text))
            matches = getattr(it, "_kw_matches", []) or []
            match_set = set(m.lower() for m in matches)
            # For each keyword group that matched this paper,
            # check that at least one of its queries has content overlap
            ok = True
            for qset in kw_query_sets:
                matching_queries = [q for q in qset if q.lower() in match_set]
                if not matching_queries:
                    continue  # this group didn't match, skip
                # Check if any matching query has ≥2 tokens in the text
                group_ok = False
                for q in matching_queries:
                    qtokens = _TOK2.findall(q.lower())
                    overlap = len(qtokens & text_tokens) if isinstance(qtokens, set) else sum(1 for t in qtokens if t in text_tokens)
                    if isinstance(qtokens, list):
                        overlap = sum(1 for t in qtokens if t in text_tokens)
                    if overlap >= 2:
                        group_ok = True
                        break
                if not group_ok:
        ...
        if filtered:
            all_items = filtered
        print(f"[deep-search] text filter: removed {len(scored)-len(filtered) if 'scored' in dir() else 0} items, "
              f"kept {len(filtered)}", flush=True)
