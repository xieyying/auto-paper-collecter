...
async def deep_search(keywords: list[str], time_range_days: int = 365,
    ...
    for i, kw_queries in enumerate(expanded_per_kw):
        ...
        for res in await asyncio.gather(*tasks):
            for it in res:
                # Track which source and query found this paper
    ...
    # Tag each paper with keywords that found it (tracked during fetch)
    for it in all_items:
        matches = getattr(it, "_kw_matches", [])
        it.topic = " & ".join(matches) if len(matches) > 1 else (matches[0] if matches else "")
