...
def _expand_sys(domain: str = "") -> str:
    field = domain or "计算机科学"
    return (
        f"你是{field}领域文献检索助手。用户给出一个研究关键词。"
        "请做"联想式扩展"：生成 5-7 个与之高度相关的英文学术检索式。"
        "不仅包括同义词、全称与常见写法，还要包括紧邻的子方向、关键技术、方法与典型应用，"
        "以便把该主题最新、最相关的论文尽量召回。"
        f"所有检索式都必须仍属于{field}领域、且与原关键词强相关，避免过度泛化跑题。"
        "严格只输出 JSON 字符串数组，不要任何其它文字。"
    )
...
def _filter_sys(domain: str = "") -> str:
    field = domain or "计算机科学"
    return (
        f"你是{field}领域文献筛选助手。给定一个研究关键词和若干论文（标题 + 来源/期刊）。"
        f"逐篇判断是否同时满足：①属于{field}领域；且 ②与该关键词主题直接相关。"
