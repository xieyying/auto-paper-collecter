...
async def deep_search(keywords: list[str], time_range_days: int = 365,
    ...
    #     Check that each original keyword appears as a phrase in the paper's
    #     title/abstract.  This ensures "protein language model" isn't matched
    #     by a paper that only contains the word "protein".
    #     Single-token keywords like "ESM2" match directly.
    if keywords and all_items and len(keywords) > 1:
        ...
        all_items = filtered or all_items  # keep filtered; if empty, fall back (but log it)
        dropped = before - len(all_items)
        if dropped:
            print(f"[deep-search] text filter: {before} -> {len(all_items)} "
